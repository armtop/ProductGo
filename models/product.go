...
type Product struct {
	...
	Productname   string  `orm:"column(productname)"`
	Model         string  `orm:"column(model);null"`
	Description   string  `orm:"column(description);null"`
	Num           int     `orm:"column(num);null"`
	Price         float64 `orm:"column(price);null"`
	ApproveStatus string  `orm:"column(approve_status);null"`
	FreightPayer  string  `orm:"column(freight_payer);null"`
	HasInvoice    bool    `orm:"column(has_invoice);null"`
}
